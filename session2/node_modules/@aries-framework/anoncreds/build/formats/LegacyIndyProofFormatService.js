...
class LegacyIndyProofFormatService {
    ...
    async getSchemas(agentContext, schemaIds) {
        ...
        for (const schemaId of schemaIds) {
            if (!schemaId.match(utils_1.legacyIndySchemaIdRegex)) {
    ...
    async getCredentialDefinitions(agentContext, credentialDefinitionIds) {
        ...
        for (const credentialDefinitionId of credentialDefinitionIds) {
            if (!credentialDefinitionId.match(utils_1.legacyIndyCredentialDefinitionIdRegex)) {
    ...
    async getRevocationStatus(agentContext, proofRequest, requestedItem, credentialInfo) {
        ...
        (0, utils_1.assertRevocationInterval)(requestNonRevoked);
        const registryService = agentContext.dependencyManager.resolve(AnonCredsRegistryService_1.AnonCredsRegistryService);
    ...
    async createProof(agentContext, proofRequest, selectedCredentials) {
        ...
        const revocationRegistries = await this.getRevocationRegistriesForRequest(agentContext, proofRequest, selectedCredentials);
    ...
    async getRevocationRegistriesForRequest(agentContext, proofRequest, selectedCredentials) {
        var _a, _b, _c;
        ...
        try {
            ...
            for (const { referent, credentialInfo, nonRevoked } of referentCredentials) {
                ...
                if (nonRevoked && credentialRevocationId && revocationRegistryId) {
                    ...
                    // Make sure the revocation interval follows best practices from Aries RFC 0441
                    (0, utils_1.assertRevocationInterval)(nonRevoked);
                    const registry = agentContext.dependencyManager
                        .resolve(AnonCredsRegistryService_1.AnonCredsRegistryService)
                    ...
                    if (!revocationRegistries[revocationRegistryId]) {
                        ...
                        const { tailsFilePath } = await (0, utils_1.downloadTailsFile)(agentContext, tailsLocation, tailsHash);
                    ...
                    const { revocationStatusList, resolutionMetadata: statusListResolutionMetadata } = await registry.getRevocationStatusList(agentContext, revocationRegistryId, (_c = nonRevoked.to) !== null && _c !== void 0 ? _c : Date.now());
    ...
    async getRevocationRegistriesForProof(agentContext, proof) {
        const revocationRegistries = {};
